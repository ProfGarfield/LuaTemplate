...
---@class combatSpec
---@field aCustomAdd? number add this to attack before multipliers are applied (negative number to subtract)\nIf aCustomAdd reduces an attack value below 1, the attack will be set to 1
---@field dCustomAdd? number add this to defense before multipliers are applied (negative number to subtract)\nIf dCustomAdd reduces a defense value below 0, the value will be set to 0
---@field aCustomMult? number multiply the attacker's strength by this much (to supply a custom bonus or penalty)
---@field dCustomMult? number multiply the defender's strength by this much (to supply a custom bonus or penalty)
---@field aAddFirepower? number add this to the attacker's firepower before any other calculations (negative number to subtract)
---@field dAddFirepower? number add this to the defender's firepower before any other calculations (negative number to subtract)
---@field [integer] number|nil Assigns a custom defense value to the terrain object where the terrainIndex is 3*16*terrain.map+3*terrain.type+terrain.resource
...
local stringNil = {["nil"]=true,["string"]=true}
local functionNil = {["nil"]=true,["function"]="function(unit)-->boolean"}
...
local combatModifierSpecificKeys = {
    ...
    attackerPedia = stringNil,
    defenderPedia = stringNil,
    attackerHelp = stringNil,
    attackerHelpActive = stringNil,
    defenderHelp = stringNil,
    defenderHelpActive = stringNil,
}
...
local reservedCombatModKeys = {["attacker"]=true,
    ["attackerDetail"]=true,
    ["defender"]=true,
    ["defenderDetail"]=true,
    ["customCheck"]=true,
    ...
    ["defenderPedia"]=true,
    ["attackerHelp"]=true,
    ["attackerHelpActive"]=true,
    ["defenderHelp"]=true,
    ["defenderHelpActive"]=true,
    }

function combatMod.processCombatModifier(combatMod)
    local internalTable = {}
    local function splitCombatModValue(value,itemKey,traitKey,pediaDescription)
        ...
        elseif type(value) == "string" then
            ...
            if pediaDescription then
                civilopedia.traitDescription(value,pediaDescription)
            end
            return
        elseif civ.isUnitType(value) then
            ...
            if pediaDescription then
                civilopedia.description(value,pediaDescription)
            end
            return
        end
    end
    if combatMod.attackerPedia and not combatMod.attacker then
        error("combatModifiers: attackerPedia is only valid if the attacker field is specified.")
    end
    if combatMod.defenderPedia and not combatMod.defender then
        error("combatModifiers: defenderPedia is only valid if the defender field is specified.")
    end
    splitCombatModValue(combatMod.attacker,"attackerUnitTypeID","attackerTraits",combatMod.attackerPedia)
    splitCombatModValue(combatMod.defender,"defenderUnitTypeID","defenderTraits",combatMod.defenderPedia)
    ...
    internalTable.attackerHelpActive = combatMod.attackerHelpActive
    internalTable.attackerHelp = combatMod.attackerHelp
    internalTable.defenderHelpActive = combatMod.defenderHelpActive
    internalTable.defenderHelp = combatMod.defenderHelp
...
local function anyDetailMatches(unit,detailItems,detailTraits)
    -- if the unit is nil, then this is checking registered rules for a help message
    if unit == nil then
        return true
    end
...
local function attackerValid(modTable,attacker)
    -- if the attacker is nil, then this is checking registered rules for a help message
...
local function defenderValid(modTable,defender)
    -- if the defender is nil, then this is checking registered rules for a help message
...
--  if the attacker is nil, then this is checking registered rules for a help
--  message for the defender.
--  if the defender is nil, then this is checking registered rules for a help
--  message for the attacker.
--  the combatModifierOverrideTable is modified in place, to make the changes to combat
--  helpLists is a table that contains the help messages for the attacker and defender
--  The helpLists table will create the following keys if they do not exist:
--  attackerHelp, defenderHelp, attackerHelpActive, and defenderHelpActive
function combatMod.applyRegisteredRules(attacker,defender,combatModifierOverrideTable,
helpLists)
    helpLists = helpLists or {}
    helpLists.attackerHelpList = helpLists.attackerHelpList or {}
    helpLists.attackerHelpActiveList = helpLists.attackerHelpActiveList or {}
    helpLists.defenderHelpList = helpLists.defenderHelpList or {}
    helpLists.defenderHelpActiveList = helpLists.defenderHelpActiveList or {}
    local attackerHelpList = helpLists.attackerHelpList
    local attackerHelpActiveList = helpLists.attackerHelpActiveList
    local defenderHelpList = helpLists.defenderHelpList
    local defenderHelpActiveList = helpLists.defenderHelpActiveList
    if doNotApplyRegisteredCombatRules then
        return
    end

    local function attackerAndDefenderValid(modTable)
        return attackerValid(modTable,attacker) and defenderValid(modTable,defender)
    end
    ...
    local function recordInfo(mTable)
        local cSpec = mTable.combatSpec
    ...
    local function loopFun(modTable)
        if attacker == nil and modTable.defenderHelpActive == nil then
            -- if the attacker is nil, then this is checking registered rules for a help message for the defender
            -- since defenderHelpActive is nil, we don't need to calculate
            -- defender bonuses for this combat modifier
            if defenderValid(modTable,defender) and modTable.defenderHelp then
                defenderHelpList[#defenderHelpList+1] = modTable.defenderHelp
            end
            return
        end
        if defender == nil and modTable.attackerHelpActive == nil then
        ...
        local defValid = defenderValid(modTable,defender)
        local modificationIsActive = false
        ...
        if attackerAndDefenderValid(modTable) and 
            ...
            recordInfo(modTable)
            modificationIsActive = true
        end
        if attacker and atkValid then
            if modTable.attackerHelpActive and modificationIsActive then
                attackerHelpActiveList[#attackerHelpActiveList+1] = modTable.attackerHelpActive
            elseif modTable.attackerHelp then
                attackerHelpList[#attackerHelpList+1] = modTable.attackerHelp
            end
        end
        if defender and defValid then
            if modTable.defenderHelpActive and modificationIsActive then
                defenderHelpActiveList[#defenderHelpActiveList+1] = modTable.defenderHelpActive
            elseif modTable.defenderHelp then
                defenderHelpList[#defenderHelpList+1] = modTable.defenderHelp
            end
        end
    end
    ...
    local function applyBuffsAndDebuffs(key)
        if type(key) == 'number' then
            if defender == nil then
                -- if the defender is nil, then this is checking registered rules for a help message for the attacker
                -- so, we don't need to apply the terrain defense value
                return
            end
